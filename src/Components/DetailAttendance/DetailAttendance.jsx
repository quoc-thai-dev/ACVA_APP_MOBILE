...
const DetailAttendance = ({status, dateLearn, timeStart, timeEnd, index}) => {
  const checkAttendance = () => {
    ...
    currentDay = Number(
      `${currentDay.getFullYear()}${String(currentDay.getMonth() + 1).padStart(
        2,
        '0',
      )}${String(currentDay.getDate()).padStart(2, '0')}`,
    );
    learnDay = Number(
      `${learnDay.getFullYear()}${String(learnDay.getMonth() + 1).padStart(
        2,
        '0',
      )}${String(learnDay.getDate()).padStart(2, '0')}`,
    );
        ...
        <View style={styles.contentContainer}>
          ...
          <View style={styles.textInfoInner}>
            <Text style={styles.textInfo}>
              {dateLearn ? formatDate(dateLearn) : ''}
            </Text>
        ...
        <View style={styles.statusContainer}>
          <View style={styles.statusInner}>
            ...
            {dateLearn ? (
              ...
              <View style={styles.textattendance}>
                <Text style={{color: 'red'}}>{t('absent')}</Text>
              </View>
            )}
          </View>
        </View>
